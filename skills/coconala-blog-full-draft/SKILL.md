...
description: End-to-end Coconala blog workflow that researches a live product page, writes a fact-checked Japanese blog article, generates a thumbnail plus two body images in the user's editable ChatGPT image chat, normalizes images under 700 KB, uploads everything to Coconala, saves only as an unpublished draft, and verifies the reopened draft. Use when the user asks to combine Coconala blog writing, thumbnail/article image generation, and live Coconala draft upload.
---
...
4. Create one new article-specific ChatGPT chat. Send the thumbnail request, body image A request, and body image B request as three separate messages. Do not use Codex ImageGen.
